...
func (rg *registry) SetNumber(regi int, vali LUint256) { // +inline-start
	newSize := regi + 1
	// +inline-call rg.checkSize newSize
...
func (ls *LState) ToInt(n int) int {
	if lv, ok := ls.Get(n).(LUint256); ok {
		if out, ok := lu256ToInt(lv); ok {
			return out
		}
		return 0
	}
...
func (ls *LState) ToUint256(n int) LUint256 {
	return LVAsUint256(ls.Get(n))
}
...
func (ls *LState) ObjLen(v1 LValue) int {
	...
	if op.Type() == LTFunction {
		...
		if ret.Type() == LTUint256 {
			if out, ok := lu256ToInt(ret.(LUint256)); ok {
				return out
			}
			return 0
		}
