...
func main() {
	...
	familyEntries := make(map[string][]lua.StdlibReleaseEntry)
	...
	for _, family := range families {
		entries := append([]lua.StdlibReleaseEntry(nil), familyEntries[family]...)
		sort.Slice(entries, func(i, j int) bool {
			return entries[i].Contract < entries[j].Contract
		})
		bundleBytes, err := lua.BuildStdlibFamilyBundlePackage(entries, sources)
		if err != nil {
			fatalf("build family bundle %s: %v", family, err)
		}
		bundleRel := filepath.ToSlash(filepath.Join("stdlib", "releases", family, family+".tor"))
		...
		bundleCatalogRel := filepath.ToSlash(filepath.Join("stdlib", "releases", family, family+".bundle.json"))
		...
		bundleDiscoveryRel := filepath.ToSlash(filepath.Join("stdlib", "releases", family, family+".bundle.discovery.json"))
		bundleAgentPkgRel := filepath.ToSlash(filepath.Join("stdlib", "releases", family, family+".bundle.agentpkg.json"))
		serviceKinds := []string{}
		capabilities := []string{}
		tags := []string{}
