...
| `session` | session issuer and terminal-trust policy | session expiry must dominate reuse; step-up threshold must be enforced before spend; revoked session must become inert immediately | fail closed on inactive or degraded session | moderate dependency on trustworthy terminal/session evidence outside chain |
