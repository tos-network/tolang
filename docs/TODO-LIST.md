...
| ID | Item | Status | Notes |
|---|---|---|---|
| V2-1 | Sponsor-aware settlement joins | Done | GTOS runtime receipts/effects now persist `sponsor`, `tos.receipt_open(...)` / `tos.settle*` accept sponsor-aware opts, and Go/TS/OpenFox consumption layers expose sponsor-linked settlement inspection end to end. Design home: `/home/tomi/gtos/docs/GTOS_SETTLEMENT_BUS_AND_RECEIPT_HOOKS.md` |
| V2-2 | `ESCROW_RELEASE_UNO` | Done | GTOS settlement bus now supports canonical `ESCROW_RELEASE_UNO`, Tolang lowering exposes `settlement.releaseEscrowUno(...)`, and openlib `ConfidentialEscrow.releaseEscrow(...)` now settles through that runtime mode with rollback coverage. Design home: `/home/tomi/gtos/docs/GTOS_SETTLEMENT_BUS_AND_RECEIPT_HOOKS.md` |
| V2-3 | Deeper confidential refund / release normalization | Done | Confidential refund/release flows now converge on one canonical settlement-bus shape across runtime state, harnesses, and openlib adoption instead of mixing ad hoc confidential transfer paths. Design home: `/home/tomi/gtos/docs/GTOS_SETTLEMENT_BUS_AND_RECEIPT_HOOKS.md` |
| V2-4 | OpenFox local-settlement to canonical-runtime bridge | Done | OpenFox settlement records now persist canonical `runtimeReceiptRef` / `runtimeSettlementRef`, CLI and operator API bridge those refs into GTOS runtime inspection, and local settlement UX no longer has to live in a separate ID domain when canonical refs are available. Design home: `/home/tomi/gtos/docs/GTOS_SETTLEMENT_BUS_AND_RECEIPT_HOOKS.md` |
| V2-5 | Richer verifier classes | Done | Protocol registry inspection now projects `verifier_class` and `proof_class` instead of only raw numeric verifier types, so `@verifiable`-adjacent runtime/client consumers can reason about stronger verifier classes without app-specific decoding. Design home: `/home/tomi/gtos/docs/GTOS_PROTOCOL_REGISTRIES.md` |
| V2-6 | Richer settlement-policy classes | Done | Pay-policy inspection now projects `policy_class` alongside numeric kinds, so settlement-backed policy routing can evolve through typed registry classes instead of hard-coding more app-specific kind tables. Design home: `/home/tomi/gtos/docs/GTOS_PROTOCOL_REGISTRIES.md` |
| V2-7 | Deeper discovery / deployment trust integration | Done | Suggested discovery cards now carry normalized `deployment_trust` hints derived from package/publisher trust, and SDK/OpenFox card types understand those hints so discovery/deployment trust is no longer limited to bare package resolution flags. Design home: `/home/tomi/gtos/docs/PACKAGE_PUBLISHING_REGISTRY.md` |

The bounded V2 cap is now complete.
