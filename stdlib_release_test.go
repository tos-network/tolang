...
func TestStdlibReleaseArtifactsAreCurrent(t *testing.T) {
	...
	byFamily := map[string][]StdlibReleaseEntry{}
	...
	for family, entries := range byFamily {
		...
		bundleWant, err := BuildStdlibFamilyBundlePackage(ordered, sources)
		if err != nil {
		...
		if string(bundleGot) != string(bundleWant) {
			t.Fatalf("stale family bundle tor for %s; rerun stdlib exporter", family)
		}
		...
		if string(catalogGot) != string(catalogWant) {
			t.Fatalf("stale family bundle catalog for %s; rerun stdlib exporter", family)
		}
		...
		if string(discoveryGotBytes) != string(discoveryWantBytes) {
			t.Fatalf("stale family bundle discovery for %s; rerun stdlib exporter", family)
		}
		...
		if string(agentGotBytes) != string(agentWantBytes) {
			t.Fatalf("stale family bundle agent package for %s; rerun stdlib exporter", family)
		}
	}
}
